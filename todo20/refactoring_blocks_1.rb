...
# compute_shipping
# compute_tax
# payment (this needs block)
# ship_goods

def pay_by_visa(order,ccn)
  pay_by(order) {|order| order.payment :type => :visa , :ccn => ccn; order.verify_payment}
end
...
def pay_by_store_credit(order, current_user)
  pay_by(order) {|order| order.payment :type => :store_credit; current_user.store_credit -= order.cost}
end
